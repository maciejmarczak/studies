    ...
    while(1) {
        ...
        if ( semop(sem_id, &take, 1) == 0 ) {
            if ( array[i] != 0 ) {
                ...
                if ( array[i] % 2 == 0 ) result = "parzysta";
                else result = "nieparzysta";

                semop(sem_id, &take_m, 1);
                array[MAX_TASKS] -= 1;
                printf( "(PID: %d\tTIMESTAMP: %u)\tSprawdzilem liczbe "
                        "%d na pozycji %d - %s. Pozostalo zadan oczekujacych: %d.\n",
                        pid, (unsigned) time(NULL), array[i], i, result, array[MAX_TASKS]);
